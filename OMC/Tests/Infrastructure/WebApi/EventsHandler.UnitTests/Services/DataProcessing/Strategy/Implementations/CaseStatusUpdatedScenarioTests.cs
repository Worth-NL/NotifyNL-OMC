...
{
    [TestFixture]
    public sealed class CaseStatusUpdatedScenarioTests
    {
        ...
        #region Test data
        private const string WhitelistedId = "1";
        private const string NotWhitelistedId = "4";
        private const string StatusTypeName = "In behandeling";

        private static CaseStatusType WhitelistedStatusType => new() { Identification = WhitelistedId, Name = StatusTypeName };
        private static CaseStatusType NotWhitelistedStatusType => new() { Identification = NotWhitelistedId };
        ...
        {
            // Arrange
            INotifyScenario scenario = ArrangeCaseUpdatedScenario_TryGetData(
                NotWhitelistedStatusType, DistributionChannels.Email);
            ...
            Assert.Multiple(() =>
            {
                ...
                string expectedErrorMessage = ApiResources.Processing_ABORT_DoNotSendNotification_Whitelist_CaseTypeId
                    .Replace("{0}", NotWhitelistedId)
                    .Replace("{1}", "ZGW_WHITELIST_ZAAKUPDATE_IDS");
            ...
            DistributionChannels invalidChannel)
        {
            // Arrange
            INotifyScenario scenario = ArrangeCaseUpdatedScenario_TryGetData(
                WhitelistedStatusType, invalidChannel);
            ...
            DistributionChannels testDistributionChannel, NotifyMethods? expectedNotifyMethod, int notifyDataCount, string expectedContactDetails)
        {
            // Arrange
            INotifyScenario scenario = ArrangeCaseUpdatedScenario_TryGetData(
                WhitelistedStatusType, testDistributionChannel);
            ...
            Assert.Multiple(() =>
            {
                ...
                string expectedSerialized =
                    $"{{" +
                      ...
                      $"\"zaak.omschrijving\":\"\"," +
                      $"\"status.omschrijving\":\"{StatusTypeName}\"" +
                    $"}}";
        ...
        {
            // Arrange
            INotifyScenario scenario = ArrangeCaseUpdatedScenario_ProcessData(true);
            ...
            NotifyMethods testNotifyMethod, int emailCount, int smsCount, int letterCount)
        {
            ...
            INotifyScenario scenario = ArrangeCaseUpdatedScenario_ProcessData(
        ...
        #region Setup
        private INotifyScenario ArrangeCaseUpdatedScenario_TryGetData(
            CaseStatusType caseStatusType, DistributionChannels testDistributionChannel)
        {
            ...
            return new CaseStatusUpdatedScenario(this._testConfiguration, this._mockedDataQuery.Object, this._mockedNotifyService.Object)
                .SetCaseStatusType(caseStatusType);
        }

        private const string SimulatedNotifyExceptionMessage = "Some NotifyClientException";

        private INotifyScenario ArrangeCaseUpdatedScenario_ProcessData(
            bool isSendingSuccessful, NotifyData? emailNotifyData = null, NotifyData? smsNotifyData = null, NotifyData? letterNotifyData = null)
        {
            ...
            return new CaseStatusUpdatedScenario(this._testConfiguration, this._mockedDataQuery.Object, this._mockedNotifyService.Object)
                .SetCaseStatusType(WhitelistedStatusType);
        }
        #endregion
